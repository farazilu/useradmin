 ...
 // the others are public (forgot, login, register, reset, noaccess)
    ...
    public function before()
    {
        ...
        if (substr($this->request->referrer(), 0, strlen($baseUrl)) == $baseUrl) {
            ...
            $processedRef = self::process_uri($urlPath);
    ...
    public static function process_uri($uri, $routes = NULL)
    {
        ...
        foreach ($routes as $name => $route) {
            ...
            if ($params = $route->matches(new Request($uri))) {
                return array(
                    'params' => $params,
                    'route' => $route
                );
    ...
    public function action_register()
    {
        ...
        if (Auth::instance()->logged_in() != false) {
            ...
            $this->redirect($this->default_redirect_url);
        }
        // Load the view
        ...
        if ($_POST) {
            ...
            if (Kohana::$config->load('useradmin')->captcha) {
                $recaptcha_resp = recaptcha_check_answer($recaptcha_config['privatekey'], $_SERVER['REMOTE_ADDR'], $_POST['recaptcha_challenge_field'], $_POST['recaptcha_response_field']);
    ...
    public function action_unregister()
    {
        ...
        $this->template->content = View::factory('user/unregister')->set('id', $id)->set('data', array(
        ...
        if ($this->request->is_ajax() && isset($_REQUEST['username'], $_REQUEST['password'])) {
            ...
            if ($_REQUEST && isset($_REQUEST['username'], $_REQUEST['password'])) {
                ...
                if (Auth::instance()->login($_REQUEST['username'], $_REQUEST['password'], Arr::get($_REQUEST, 'remember', false) != false)) {
                    ...
                    // Get errors for display in view
                    $validation = Validation::factory($_REQUEST)->rule('username', 'not_empty')->rule('password', 'not_empty');
                    if ($validation->check()) {
    ...
    function action_reset()
    {
        ...
        if (isset($_REQUEST['reset_token']) && isset($_REQUEST['reset_email'])) {
            ...
            if ((strlen($_REQUEST['reset_token']) == 32) && (strlen(trim($_REQUEST['reset_email'])) > 1)) {
                $user = ORM::factory('User')->where('email', '=', $_REQUEST['reset_email'])
                    ->and_where('reset_token', '=', $_REQUEST['reset_token'])
                    ->find();
                // The admin password cannot be reset by email
                if ($user->has('roles', ORM::factory('Role', array(
                    'name' => 'admin'
                )))) {
                    ...
                    if (is_numeric($user->id) && ($user->reset_token == $_REQUEST['reset_token'])) {
                        ...
                        Message::add('success', '<p>' . __('Your password has been reset to: ":password".', array(
                            ':password' => $password
                        )) . '</p><p>' . __('Please log in below.') . '</p>');
                        $this->redirect('user/login?username=' . $user->username);
                    }
            }
        }
    ...
    function action_change_password()
    {
        ...
        if (! empty($_POST) && is_numeric($id)) {
            ...
            if ($validate) {
                ...
                Message::add('success', __('Values saved.'));
                // redirect and exit
                $this->redirect('user/index'); // index will redir ya whereever you need
                return;
            } else {
    ...
    function action_provider()
    {
        ...
        if (is_object($provider)) {
            $this->redirect($provider->redirect_url('/user/provider_return/' . $provider_name));
            return;
        }
    ...
    function action_associate()
    {
        ...
        if (Auth::instance()->logged_in()) {
            if (isset($_POST['confirmation']) && $_POST['confirmation'] == 'Y') {
                $provider = Provider::factory($provider_name);
                if (is_object($provider)) {
                    $this->redirect($provider->redirect_url('/user/associate_return/' . $provider_name));
                    return;
                } else {
                    Message::add('error', 'Provider is not enabled; please select another provider or log in normally.');
                    $this->redirect('user/login');
    ...
    function action_provider_return()
    {
        ...
        if ($provider->verify()) {
            ...
            $user_identity = ORM::factory('User_Identity')->where('provider', '=', $provider_name)
    ...
    /**
     * Media routing code.
     * Allows lazy users to load images via Kohana. See also: init.php.
